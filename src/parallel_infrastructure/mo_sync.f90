...
!-----------------------------------------------------------------------
   ...
   isum_2 = 0_i8
   ! Get the maximum absolute value of all numbers.
   !$ACC PARALLEL DEFAULT(PRESENT) COPY(abs_max) REDUCTION(MAX: abs_max) ASYNC(1) IF(lzacc)
   ...
   ! Sum up all numbers as scaled integers
   !$ACC PARALLEL DEFAULT(PRESENT) COPY(isum_1, isum_2) REDUCTION(+: isum_1, isum_2) ASYNC(1) IF(lzacc)
